...
namespace aa {
    ...
    public: 
        ...
        void fixedHorizontalOriginRotation(float angleChangex, cy::Vec3f  axis) {
            cy::Matrix4f rotX = cy::Matrix4f::Rotation(axis, angleChangex);
            
            normal = (rotX * cy::Vec4f(normal, 0.0f)).XYZ().GetNormalized();
            up = (rotX * cy::Vec4f(up, 0.0f)).XYZ().GetNormalized();
            right = (rotX * cy::Vec4f(right, 0.0f)).XYZ().GetNormalized();
...
// FINISH SETTING ORTHOGRAPHIC PROJECTION
        cy::Matrix4f OrthographicProjection(float left, float right, float bottom, float top, float near, float far)
        {
            cy::Matrix4f ortho;
            ortho.SetIdentity(); 
            // Column 0:
